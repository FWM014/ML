...
def diagnose(train_score: float, val_score: float, target: float = 0.95, gap_tol: float = 0.05) -> str:
    ...
    """
    # TODO: three if-branches in the order given: gap first, then the target, else "ok"
    raise NotImplementedError("Task: diagnose")
...
def degree_sweep(X: np.ndarray, y: np.ndarray, degrees=(1, 2, 3, 5, 9, 12), seed: int = 0) -> tuple[dict, int]:
    ...
    """
    # TODO: validation_curve(model, X, y, param_name="polynomialfeatures__degree", param_range=..., cv=KFold(...), scoring="neg_mean_squared_error"); mean over axis=1 and negate
    raise NotImplementedError("Task: degree_sweep")
...
def ridge_shrinkage(X: np.ndarray, y: np.ndarray, alphas=(0.01, 1, 10, 100)) -> dict:
    ...
    """
    # TODO: for each alpha: make_pipeline(StandardScaler(), Ridge(alpha=a)).fit(X, y); coef = model.named_steps["ridge"].coef_; np.linalg.norm(coef)
    raise NotImplementedError("Task: ridge_shrinkage")
...
def lasso_sparsity(X: np.ndarray, y: np.ndarray, alphas=(0.01, 1, 5, 20)) -> dict:
    ...
    """
    # TODO: same pattern with Lasso(alpha=a, max_iter=20000); int((coef != 0).sum())
    raise NotImplementedError("Task: lasso_sparsity")
...
def cv_accuracy(X: np.ndarray, y: np.ndarray, C: float = 0.1, seed: int = 0, n_splits: int = 5) -> dict:
    ...
    """
    # TODO: build the pipeline; cv = StratifiedKFold(n_splits, shuffle=True, random_state=seed); scores = cross_val_score(model, X, y, cv=cv, scoring="accuracy")
    raise NotImplementedError("Task: cv_accuracy")
...
def learning_curve_summary(model, X: np.ndarray, y: np.ndarray, sizes=(0.1, 0.25, 0.5, 0.75, 1.0),
    ...
    """
    # TODO: n, tr, va = learning_curve(...); average each over axis=1; gap = train[-1] - val[-1]; rising = val[-1] - val[-2] > 0.005
    raise NotImplementedError("Task: learning_curve_summary")
...
def model_report(X: np.ndarray, y: np.ndarray, seed: int = 0) -> dict:
    ...
    """
    # TODO: for each candidate: cross_val_score(clone(model), ...).mean(); clone(model).fit(X, y).score(X, y); diagnose(train_score, cv_mean); recommended = max by cv_mean
    raise NotImplementedError("Task: model_report")
