...
def permutation_importance_manual(model, X: pd.DataFrame, y: np.ndarray, n_repeats: int = 20,
    ...
    """
    # TODO: baseline log-loss once; for each column loop n_repeats: X.copy(), column = rng.permutation(values), log-loss difference
    raise NotImplementedError("Task: permutation_importance_manual")
...
def partial_dependence_manual(model, X: pd.DataFrame, feature: str, grid: np.ndarray) -> np.ndarray:
    ...
    """
    # TODO: loop over grid values, overwrite the column on a copy of X, average predict_proba[:, 1]
    raise NotImplementedError("Task: partial_dependence_manual")
...
def shapley_values(v: dict, players: list) -> dict:
    ...
    """
    # TODO: for each player, loop k in range(n) and S in combinations(others, k); weight = k!(n-k-1)!/n!
    raise NotImplementedError("Task: shapley_values")
...
def tree_shap_explain(rf, X_rows: pd.DataFrame) -> dict:
    ...
    """
    # TODO: shap.TreeExplainer(rf); sv = explainer.shap_values(X_rows); pick class 1; expected_value[1]
    raise NotImplementedError("Task: tree_shap_explain")
...
def counterfactual(model, x_row: pd.Series, X_ref: pd.DataFrame, target_class: int = 1,
    ...
    """
    # TODO: nested loop over X_ref.columns and steps; build pd.DataFrame([x_row]) copies; keep min |step|
    raise NotImplementedError("Task: counterfactual")
...
def fairness_metrics(y_true: np.ndarray, y_pred: np.ndarray, group: np.ndarray) -> dict:
    ...
    """
    # TODO: loop over np.unique(group), boolean masks, then max/min of the two dicts
    raise NotImplementedError("Task: fairness_metrics")
