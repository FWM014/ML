...
The export has the usual problems: the `amount` column contains the word "pending" for
unsettled orders, so pandas reads it as text, and a few customers in the orders are
missing from the CRM table.
...
def make_orders_csv(path: str | None = None, seed: int = 0, n: int = 400) -> str:
    ...
    Dirt: a few "pending" strings in `amount` (the column will read as text), dates as strings.
    """
    rng = np.random.default_rng(seed)
    os.makedirs(DATA_DIR, exist_ok=True)
    path = path or os.path.join(DATA_DIR, "ch04_orders.csv")
    amount = rng.gamma(3, 30, n).round(2).astype(object)
    amount[rng.choice(n, 6, replace=False)] = "pending"
    orders = pd.DataFrame({
...
def load_orders(path: str) -> pd.DataFrame:
    ...
    - `amount` must become float64: the "pending" strings become NaN (pd.to_numeric(errors="coerce")).
