...
def make_overfit_data(seed: int = 0, n: int = 160, d: int = 40):
    """A tiny, wide, noisy table: 40 features, only 4 informative, 160 rows.

    A large MLP memorizes it easily -> big train/val gap. Do not modify.
    """
    X, y = make_classification(n_samples=n, n_features=d, n_informative=4, n_redundant=0,
                               n_clusters_per_class=2, flip_y=0.15, class_sep=0.8, random_state=seed)
...
def regularization_gap(seed: int = 0, epochs: int = 30) -> dict:
    """§7: on make_overfit_data(seed), train two MLPs of the same size, hidden=(128, 128):

    - "plain":       dropout=0.0, weight_decay=0.0
    - "regularized": dropout=0.5, weight_decay=0.1
    both with train_with_early_stopping(lr=1e-2, max_epochs=epochs, patience=epochs
    ...
    result = {}
    for name, p_drop, wd in [("plain", 0.0, 0.0), ("regularized", 0.5, 0.1)]:
        torch.manual_seed(seed)
        model = build_mlp(X_tr.shape[1], (128, 128), 1, dropout=p_drop)
        hist = train_with_early_stopping(model, X_tr, y_tr, X_va, y_va, lr=1e-2, max_epochs=epochs,
