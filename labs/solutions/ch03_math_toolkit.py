...
def batch_predict(X: np.ndarray, W: np.ndarray, b: np.ndarray) -> np.ndarray:
    ...
    """
    X, W, b = np.asarray(X, dtype=float), np.asarray(W, dtype=float), np.asarray(b, dtype=float)
    if X.ndim != 2 or W.ndim != 2 or X.shape[1] != W.shape[0]:
        raise ValueError(f"cannot multiply X of shape {X.shape} by W of shape {W.shape}: "
                         f"inner dimensions must match")
    if b.shape != (W.shape[1],):
        raise ValueError(f"bias shape {b.shape} must be ({W.shape[1]},) to match W of shape {W.shape}")
    return X @ W + b            # (n, d) @ (d, k) -> (n, k); b broadcasts over rows
...
def bayes_posterior(prior: float, sensitivity: float, specificity: float) -> dict:
    ...
    posterior = prior * sensitivity / p_pos
    return {
        "p_positive": float(p_pos),
        "posterior": float(posterior),
        "per_1000": {"true_positives": float(1000 * prior * sensitivity),
                     "false_positives": float(1000 * (1 - prior) * (1 - specificity))},
    }
...
def information_gain(y: np.ndarray, mask: np.ndarray) -> float:
    ...
    """
    y, mask = np.asarray(y).astype(int), np.asarray(mask, dtype=bool)
    n = len(y)

    def h(labels):
        if len(labels) == 0:
            return 0.0
        return entropy(np.bincount(labels) / len(labels))

    left, right = y[mask], y[~mask]
    after = len(left) / n * h(left) + len(right) / n * h(right)
    return max(0.0, h(y) - after)
...
def gradient_descent(f, grad, w0: np.ndarray, lr: float = 0.1, n_steps: int = 50,
    ...
    if np.max(np.abs(np.asarray(grad(w)) - numerical_gradient(f, w))) > check_tol:
        raise ValueError("gradient check failed: analytic and numerical gradients disagree")
    losses = []
    for _ in range(n_steps):
        w = w - lr * np.asarray(grad(w), dtype=float)
