...
classifier that beats 80 % cross-validated accuracy on an inline ticket corpus, cosine
...
def make_ticket_corpus() -> tuple[list[str], list[str]]:
    """48 labelled support tickets, 16 per class (billing / bug / feature). Do not modify."""
    billing = [
    ...
    return billing + bug + feature, ["billing"] * 16 + ["bug"] * 16 + ["feature"] * 16
...
def tfidf_ticket_classifier(texts: list[str], labels: list[str], cv: int = 4) -> dict:
    ...
    Returns {"cv_accuracy": float, "pipeline": fitted Pipeline, "top_terms": {label: [3 terms]}}.
    """
    pipe = Pipeline([
    ...
    top = {cls: [vocab[i] for i in row.argsort()[-3:][::-1]] for cls, row in zip(pipe["lr"].classes_, pipe["lr"].coef_)}
...
def retrieve_best_chunk(query: str, chunks: list[str]) -> tuple[int, float]:
    ...
    Example: for HELP_CENTRE chunked with size=40, overlap=10, the query
    "how long does a refund take" returns the chunk containing "5 business days".
    """
...
if __name__ == "__main__":
    ...
    chunks = chunk_text(HELP_CENTRE, size=40, overlap=10)
    i, s = retrieve_best_chunk("how long does a refund take", chunks)
    print(f"RAG: {len(chunks)} chunks; best for 'how long does a refund take' = #{i} (cos {s:.2f}): {chunks[i][:80]}...")
