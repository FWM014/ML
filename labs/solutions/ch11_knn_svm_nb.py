...
def distance_concentration(n: int = 1000, dims=(1, 2, 10, 100, 1000), seed: int = 0) -> pd.DataFrame:
    ...
    for d in dims:
        ...
        rows.append({"d": int(d), "nearest": float(dist.min()), "farthest": float(dist.max()),
                     "ratio": float(dist.min() / dist.max())})
...
def knn_predict(X_tr, y_tr, X_q, k: int = 5) -> np.ndarray:
    ...
    dist = np.sqrt(((X_q[:, None, :] - X_tr[None, :, :]) ** 2).sum(axis=2))
    nearest = np.argsort(dist, axis=1, kind="stable")[:, :k]
    preds = np.empty(len(X_q), dtype=int)
    for i, idx in enumerate(nearest):
        votes = np.bincount(y_tr[idx], minlength=y_tr.max() + 1)
        preds[i] = int(np.argmax(votes))       # argmax returns the first (smallest) label on ties
    return preds
...
def scaling_effect(X, y, k: int = 5, seed: int = 0) -> dict[str, float]:
    ...
    scaled = cross_val_score(make_pipeline(StandardScaler(), KNeighborsClassifier(n_neighbors=k)), X, y, cv=cv).mean()
    return {"raw": float(raw), "scaled": float(scaled)}
...
def tune_svm(X_tr, y_tr, cv: int = 5):
    ...
    search = GridSearchCV(pipe, grid, cv=cv, scoring="accuracy", n_jobs=1).fit(X_tr, y_tr)
    res = pd.DataFrame(search.cv_results_)
    table = res.pivot(index="param_svm__C", columns="param_svm__gamma", values="mean_test_score")
    return dict(search.best_params_), table, search
...
def naive_bayes_from_scratch(docs: list[str], labels, queries: list[str], alpha: float = 1.0) -> np.ndarray:
    ...
    C = vec.transform(docs).toarray()
    Q = vec.transform(queries).toarray()
    V = C.shape[1]
    log_prior = np.log(np.array([(y == 0).mean(), (y == 1).mean()]))
    log_lik = np.empty((2, V))
    for c in (0, 1):
        wc = C[y == c].sum(axis=0)
        log_lik[c] = np.log((wc + alpha) / (wc.sum() + alpha * V))
    scores = Q @ log_lik.T + log_prior           # (n_queries, 2)
...
def compare_models(X, y, seed: int = 1) -> pd.DataFrame:
    ...
    for name, m in models.items():
        ...
        rows.append({"model": name, "auc_mean": float(s.mean()), "auc_std": float(s.std())})
