...
def class_balance(y) -> dict:
    ...
    """
    # TODO: pd.Series(y).value_counts() gives counts; divide by len for fractions; the baseline is the largest fraction
    raise NotImplementedError("Task: class_balance")
...
def missingness_report(df: pd.DataFrame) -> pd.DataFrame:
    ...
    other        0          0.00
    """
    # TODO: df.isna().sum() -> a Series; build the DataFrame and sort_values("frac_missing", ascending=False, kind="stable")
    raise NotImplementedError("Task: missingness_report")
...
def fix_sentinels(df: pd.DataFrame, column: str, sentinel: float = -999) -> pd.DataFrame:
    ...
    """
    # TODO: copy the frame; use Series.where(cond) to blank the sentinel; the flag is isna().astype(int)
    raise NotImplementedError("Task: fix_sentinels")
...
def normalise_and_dedupe(df: pd.DataFrame, cat_columns: list[str]) -> pd.DataFrame:
    ...
    """
    # TODO: loop over cat_columns applying .str.strip().str.lower(); then drop_duplicates().reset_index(drop=True)
    raise NotImplementedError("Task: normalise_and_dedupe")
...
def find_suspect_columns(df: pd.DataFrame, target: str, id_ratio: float = 0.95,
    ...
    """
    # TODO: loop over columns; nunique()/len(df) and dtype.kind for IDs; Series.corr for the notna indicator and for numeric values
    raise NotImplementedError("Task: find_suspect_columns")
...
def time_split(df: pd.DataFrame, date_col: str, test_frac: float = 0.2):
    ...
    """
    # TODO: sort_values(date_col); cut = quantile(1 - test_frac); boolean masks < cut and >= cut
    raise NotImplementedError("Task: time_split")
...
def stratified_split(df: pd.DataFrame, target: str, fracs=(0.6, 0.2, 0.2), seed: int = 0):
    ...
    """
    # TODO: two calls to sklearn.model_selection.train_test_split with stratify=; the second test_size is fracs[1] / (fracs[0] + fracs[1])
    raise NotImplementedError("Task: stratified_split")
