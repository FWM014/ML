...
def recommend_items(R: np.ndarray, user: int, k: int = 2, top_n: int = 3) -> list[tuple[int, float]]:
    ...
    """
    # TODO: mask = R > 0; user means with np.nanmean; double loop for S; loop unrated items for predictions
    raise NotImplementedError("Task: recommend_items")
...
def mf_holdout_rmse(R: np.ndarray, k: int, holdout_frac: float = 0.2, seed: int = 0) -> dict:
    ...
    """
    # TODO: known = np.argwhere(R > 0); rng.choice(len(known), n_hold, replace=False); np.linalg.svd
    raise NotImplementedError("Task: mf_holdout_rmse")
...
def precision_at_k(ranked: list, relevant: set, k: int) -> float:
    ...
    """
    # TODO: len(set(ranked[:k]) & relevant) / k
    raise NotImplementedError("Task: precision_at_k")
...
def anomaly_precision_at_k(X: np.ndarray, y_true: np.ndarray, k: int = 30, contamination: float = 0.03,
    ...
    """
    # TODO: IsolationForest(...).fit(X); scores = -det.score_samples(X); np.argsort(-scores)[:k]
    raise NotImplementedError("Task: anomaly_precision_at_k")
...
def t_learner_uplift(X_tr, T_tr, Y_tr, X_te, T_te, Y_te, seed: int = 0) -> dict:
    ...
    """
    # TODO: boolean masks T_tr == 1 / == 0; np.argsort(-tau_hat); slice(i*d, (i+1)*d) per decile
    raise NotImplementedError("Task: t_learner_uplift")
...
def simulate_bandit(p_true: np.ndarray, policy: str, horizon: int = 3000, seed: int = 0,
    ...
    """
    # TODO: n, s = zeros(K); loop t; choose a per policy; r = rng.random() < p_true[a]; accumulate regret
    raise NotImplementedError("Task: simulate_bandit")
...
def q_learning_gridworld(n_episodes: int = 500, seed: int = 1, alpha: float = 0.2, gamma: float = 0.9,
    ...
    Return {"Q": array (size, size, 4), "policy": array (size, size) of argmax actions,
            ...
            "reached_goal": bool}
    """
    # TODO: moves dict; a step(s, a) helper; two nested loops for training; then a greedy walk
    raise NotImplementedError("Task: q_learning_gridworld")
